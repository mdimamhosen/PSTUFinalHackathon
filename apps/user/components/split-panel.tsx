      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Input
            ...
            placeholder="Debtor usernames (comma separated)"
          />
          <Input
            value={perShare}
            onChange={(e) => setPerShare(e.target.value)}
            placeholder="Each share ৳"
          />
          <Button
            className="w-full"
            loading={pending}
              ...
              <div key={String(bill.id)} className="rounded-lg border p-3 text-sm">
                ...
                {shares.map((s) => {
                  ...
                  return (
                    <div
                      key={String(s.id)}
                      className="mt-2 flex items-center justify-between rounded bg-slate-50 p-2"
                    >
                      <span>
                      ...
                      {mine && String(s.status) === "PENDING" && String(s.kind) === "DEBTOR" && (
                        <div className="flex gap-1">
                          <Button
                            ...
                            onClick={() =>
                              start(async () => {
                                const r = await paySplitShareAction(String(bill.id), String(s.id));
